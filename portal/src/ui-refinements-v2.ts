...
async function openGuide(main:Element){
  ...
  const chips=make('div','v4-task-guide-kinds');const kinds=guide?[...guide.appliesTo,...(guide.conditionalFor||[])]:['casa_nova','predio_residencial','comercial_novo','reforma','ampliacao'];[...new Set(kinds)].forEach((k:any)=>{const c=make('i',guide?.conditionalFor?.includes(k)?'conditional':'applies',`${WORK_KIND_LABELS[k]}${guide?.conditionalFor?.includes(k)?' · depende do escopo':''}`);chips.append(c)});app.append(chips);if(current){const cur=make('div',`v4-task-guide-current ${applicability?.status||''}`,`Obra atual: ${WORK_KIND_LABELS[current]} · ${applicability?.label||'revisar aplicabilidade'}`);app.append(cur)}dialog.append(app)
