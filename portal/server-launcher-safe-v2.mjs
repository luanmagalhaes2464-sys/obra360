...
app.post('/api/os/projects/:id/copilot/interpret',auth,async(req,res)=>{
  ...
  const looksLikeQuestion = transcript.includes('?') || /^(o que|qual|quais|quanto|quantos|como|quando|onde|quem|por que|porque|tem|existe|estamos|estou|faltam|falta|proximo|proxima|voce|posso|pode|me diga|me fale|pesquise|procure)/.test(low) || /\b(acesso a internet|acesso internet|pesquisa na internet|consultar internet|pesquisar na web|google|na internet|na web|norma atual|versao atual|atualizado|atualizada)\b/.test(low)
  ...
  if(asksInternetCapability || asksSystemCapability){
    ...
    analysis={summary:transcript,action:'query',suggestions:[],safety:[],nextStep:b.summary.nextTask?.title||'',answer:internet
      ...
      : 'Eu consigo consultar e alterar os dados permitidos desta obra no sistema, mas a pesquisa na internet está indisponível porque a chave do Gemini não está configurada.',createLabel:'',confidence:99,mode:internet?'gemini-web':'grounded'}
  ...
  if(process.env.GEMINI_API_KEY && looksLikeQuestion){
    try{
      ...
      if(gr.ok){
        ...
        if(answer){analysis={summary:transcript,action:'query',suggestions:[],safety:[],nextStep:b.summary.nextTask?.title||'',answer,createLabel:'',confidence:92,mode:sources.length?'gemini-web':'gemini-system',sources};return res.json({analysis})}
      }else{const detail=await gr.text().catch(()=>String(gr.status));console.error('matinho-web',gr.status,detail.slice(0,400))}
  ...
  if(process.env.GEMINI_API_KEY && !looksLikeQuestion){
    try{
      ...
      if(gr.ok){
        ...
        const ids=Array.isArray(parsed.taskIds)?parsed.taskIds.map(Number):[];const valid=ids.map(taskId=>tasks.find(t=>t.id===taskId)).filter(Boolean).slice(0,8)
        ...
        analysis={summary:transcript,action,suggestions:valid.map((t,i)=>({id:t.id,title:t.title,stageKey:t.stage_key,discipline:t.discipline,confidence:Math.max(55,Math.min(99,Number(parsed.confidence)||90)-i*2)})),safety:Array.isArray(parsed.safety)?parsed.safety.map(String).slice(0,6):analysis.safety||[],nextStep:b.summary.nextTask?.title||'',answer:String(parsed.answer||''),createLabel:String(parsed.createLabel||analysis.createLabel||''),confidence:Math.max(0,Math.min(100,Number(parsed.confidence)||90)),mode:'gemini-system'}
      }
